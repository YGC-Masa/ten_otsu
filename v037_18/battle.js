...
(function () {
  ...
  function beginTimeSaleCountdown() {
    ...
    state.rush = false;
    state.surface = null;
    state.lastActionText = "第1WAVE終了！タイムセール準備中。";
    stopLoop();

    const steps = [
      { title: "3", sub: "タイムセール準備", ms: 650 },
      { title: "2", sub: "売場加速中", ms: 650 },
      { title: "1", sub: "呼び込み開始", ms: 650 },
      { title: "タイムセール開始！", sub: "15秒勝負", ms: 760 }
    ];
  ...
  function onStaffDoubleTap(staffId) {
    ...
    if (s.skill < 100) {
      // v037_18: ダブルタップ時に必殺不可でも、通常接客が可能なら通常クリックとして処理する
  ...
  function render() {
    ...
    const statusText = state.running ? (state.timeSaleActive ? "タイムセール中" : "営業中") : state.finished ? "終了" : state.timeSaleCountdown ? "タイムセール準備中" : "待機中";
    ...
    root.innerHTML = `
      <div class="battle-stage ${state.running ? "is-running" : ""} ${state.rush ? "is-rush" : ""}" style="--battle-bg-url: url(${getBattleBackground(state.battleBgId).path});">
        <section class="battle-hud">
          ...
          ${state.timeSaleActive ? `<div class="battle-timesale-banner">＞＞＞タイムセール実施中！＜＜＜</div>` : ""}
        </section>
  ...
  function renderCutinOverlay() {
    ...
    const portrait = state.cutin.image ? `
      <div class="battle-cutin-portrait-fixed" style="--cutin-color:${state.cutin.color}; opacity:${opacity};">
        <img src="${escapeHtml(state.cutin.image)}" alt="">
      </div>
    ` : "";

    const text = `
      <div class="battle-cutin-text-layer" style="--cutin-color:${state.cutin.color}; opacity:${opacity};">
    ...
    return `
      <div class="battle-cutin ${state.cutin.image ? "has-fixed-portrait" : ""}" style="pointer-events:none;">
        ${portrait}
        ${text}
      </div>
    `;
