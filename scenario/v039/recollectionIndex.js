window.TENOTSU_RECOLLECTION_STORY_INDEX = [
  {
    "id": "ai_spring_book_bread",
    "title": "桜木陰のしおり",
    "type": "normal",
    "category": "season_memory",
    "season": "spring",
    "placeId": "spring_park",
    "placeName": "春の公園",
    "characters": [
      "ab"
    ],
    "characterNames": [
      "速水川 藍"
    ],
    "scenario": "scenario/v039/events/ai_spring_book_bread.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "order": 101,
    "summary": "春の公園で、藍と本とパンをきっかけに静かな時間を過ごす季節イベント。",
    "rawTitle": "桜木陰のしおり",
    "locationName": "春の公園",
    "albumTab": "spring"
  },
  {
    "id": "hina_spring_bento",
    "title": "春の公園でのお弁当タイム",
    "type": "normal",
    "category": "season_memory",
    "season": "spring",
    "placeId": "spring_park",
    "placeName": "春の公園",
    "characters": [
      "aa"
    ],
    "characterNames": [
      "星野 緋奈"
    ],
    "scenario": "scenario/v039/events/hina_spring_bento.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "order": 102,
    "summary": "春の公園で、緋奈とお弁当を囲んで過ごす季節イベント。",
    "rawTitle": "春の公園でのお弁当タイム",
    "locationName": "春の公園",
    "albumTab": "spring"
  },
  {
    "id": "kogane_natsumi_marinpia",
    "title": "マリンピアの海中トンネル",
    "type": "normal",
    "category": "town_encounter",
    "season": "summer",
    "placeId": "marinpia_aqua_tunnel",
    "placeName": "マリンピア・海中トンネル",
    "characters": [
      "ad",
      "bc"
    ],
    "characterNames": [
      "小麦沢 こがね",
      "日向 夏海"
    ],
    "scenario": "scenario/v039/events/kogane_natsumi_marinpia.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "encounter": {
      "enabled": true,
      "staminaCost": 10,
      "revealItem": "visitor_scope",
      "seasonItem": "season_ticket"
    },
    "order": 105,
    "summary": "こがねと夏海と一緒に、マリンピアの海中トンネルを見に行く夏の外回りイベント。",
    "albumTab": "summer"
  },
  {
    "id": "sample_hina_kogane_new_juice_002",
    "title": "事務所にて緋奈＆こがね：新作ジュースは何の味？",
    "type": "normal",
    "category": "office",
    "placeId": "hidamari_office",
    "placeName": "ひだまりストア・事務所",
    "characters": [
      "aa",
      "ad",
      "ac"
    ],
    "characterNames": [
      "星野 緋奈",
      "小麦沢 こがね",
      "草壁 翠"
    ],
    "scenario": "scenario/v039/events/sample_hina_kogane_new_juice_002.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "encounter": {
      "enabled": false
    },
    "order": 106,
    "summary": "事務所の新作ジュースをきっかけに、緋奈が照れて、こがねが無邪気にかき回し、翠が締める日常会話。",
    "season": "other",
    "albumTab": "other"
  },
  {
    "id": "c10002",
    "title": "一度くらいなら……事務所にて（翠）",
    "type": "normal",
    "category": "daily",
    "season": "other",
    "placeId": "hidamari_office",
    "placeName": "ひだまりストア・事務所",
    "characters": [
      "ac"
    ],
    "characterNames": [
      "草壁 翠"
    ],
    "scenario": "scenario/v039/events/c10002.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "order": 107,
    "summary": "事務所で翠と過ごす日常イベント。",
    "rawTitle": "一度くらいなら……事務所にて（翠）",
    "locationName": "ひだまりストア・事務所",
    "albumTab": "other"
  },
  {
    "id": "common_opening_kadenseijin_raid_002_all_cast",
    "title": "共通オープニング：家電星人襲来・全員登場版",
    "type": "normal",
    "category": "event",
    "season": "event",
    "placeId": "hidamari_store",
    "placeName": "ひだまりストア",
    "characters": [],
    "characterNames": [],
    "scenario": "scenario/v039/events/common_opening_kadenseijin_raid_002_all_cast.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "order": 108,
    "summary": "倉庫在庫消失事件をきっかけに、ひだまりストア全員が家電星人と出会う共通導入イベント。家電星人の謝罪、金属生成、売上黒字化を経て、店長が家電星人を接客対象として迎える流れ。",
    "rawTitle": "共通オープニング：家電星人襲来・全員登場版",
    "locationName": "ひだまりストア",
    "albumTab": "event"
  },
  {
    "id": "event_black_kadenseijin_battle_unlock_003_ayame_line_fix",
    "title": "イベントバトル解放：ブラック家電星人を助けて",
    "type": "normal",
    "category": "event",
    "season": "event",
    "placeId": "hidamari_store",
    "placeName": "ひだまりストア",
    "characters": [
      "ak"
    ],
    "characterNames": [
      "紫藤 彩愛"
    ],
    "scenario": "scenario/v039/events/event_black_kadenseijin_battle_unlock_003_ayame_line_fix.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "order": 109,
    "summary": "ブラック家電星人を助けるイベントバトル解放イベント。",
    "rawTitle": "イベントバトル解放：ブラック家電星人を助けて",
    "locationName": "ひだまりストア",
    "albumTab": "event"
  },
  {
    "id": "biribiri_intro_rival_battle_unlock_003_flow_fix",
    "title": "ビリビリ電機登場：初来店と販売勝負",
    "type": "normal",
    "category": "event",
    "season": "event",
    "placeId": "hidamari_store",
    "placeName": "ひだまりストア",
    "characters": [
      "ba",
      "bb",
      "bc"
    ],
    "characterNames": [
      "天神 小春",
      "霧島 真冬",
      "日向 夏海"
    ],
    "scenario": "scenario/v039/events/biribiri_intro_rival_battle_unlock_003_flow_fix.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "order": 110,
    "summary": "ひだまりストア店頭に、来店チャイムとともにビリビリ電機の小春・真冬・夏海が来店。緋奈・こがね・翠が競合他社の敵情視察だと反応し、売上の秘密を探ろうとする小春たちとコミカルに衝突。挑発合戦の末、小春が販売勝負を宣言し、対戦モードが解放される。",
    "rawTitle": "ビリビリ電機登場：初来店と販売勝負",
    "locationName": "ひだまりストア",
    "albumTab": "event"
  },
  {
    "id": "shop_exchange_intro_sakuya",
    "title": "アイテム交換所解放：朔夜の交換カウンター",
    "type": "normal",
    "category": "event",
    "season": "event",
    "placeId": "exchange_counter",
    "placeName": "交換カウンター",
    "characters": [
      "da"
    ],
    "characterNames": [
      "宵闇 朔夜"
    ],
    "scenario": "scenario/v039/events/shop_exchange_intro_sakuya.json",
    "version": "v039_228",
    "unlock": {
      "type": "always"
    },
    "order": 111,
    "summary": "朔夜の交換カウンターとアイテム交換所を解放する導入イベント。",
    "rawTitle": "アイテム交換所解放：朔夜の交換カウンター",
    "locationName": "交換カウンター",
    "albumTab": "event"
  }
];
