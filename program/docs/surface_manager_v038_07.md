...
v038_27の `MutationObserver` は、メニュー表示後にDOM再構築を繰り返してハングアップする可能性があったため廃止。以後は起動時・メニュークリック時・API呼び出し時だけ必要な補正を行う。
...
HAR上で `script.js` の load handler から `000start.json` が複数回fetchされ、surfaceManagerの事務所表示後にタイトル/ランダム表示系が再実行されていた。v038_27では surface takeover中の初期 `loadScenario(currentScenario)` を停止し、起動表示を surfaceManager へ完全委譲する。`000start.json` 自体も安全なoffice指定のみに変更。

## v038_27 追記

通常PCブラウザで `#rotate-warning` が誤表示されるため、`updateOrientationWarning()` を追加。幅900px以上、または hover/pointer fine 環境では横画面OKとして警告を非表示。狭いportraitかつタッチ系入力時のみ表示する。

## v038_27 追記

`randomShows.js` がindexで先に読まれていたため、takeover判定がfalseのまま旧ランダム表示系を準備していた。indexにpreflightを追加し、`bootRescue.js` を `randomShows.js` より前へ移動。`randomShows.js` はsurface takeover中、視覚DOMを生成せず、既存DOMも削除する。

## v038_27 追記

旧 `#char-layer` が事務所表示後に遅延復活して後ろのスロットとして表示されるため、office/shop/members/settings中は `#char-layer` を完全非表示にする。さらに `officeSlotGuard` を短時間走らせ、旧random/showlist/waitからの遅延DOM生成を掃除する。

## v038_27 追記

旧showlist由来のピンク右メニューが点滅時に復活するため、旧メニュー系DOMを `hardHideLegacyMenus()` とCSSで強制非表示。style/class変更で復活した場合に備えて、限定的なMutationObserverで旧メニューだけを即時非表示にする。`#dialogue-box` はz-index 2200へ上げ、前面に表示。

## v038_27 追記

旧フレームワークの中核 `script.js` に直接ガードを入れ、surface takeover中は `000start` / `randomimageson` / `randomtexts` / `showlist` / `loadList` / `showList` を旧DOM描画へ進ませない。旧全DOM `MutationObserver` も takeover中は開始しない。
