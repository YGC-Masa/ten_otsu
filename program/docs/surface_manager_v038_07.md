...
v038_23の `MutationObserver` は、メニュー表示後にDOM再構築を繰り返してハングアップする可能性があったため廃止。以後は起動時・メニュークリック時・API呼び出し時だけ必要な補正を行う。
...
HAR上で `script.js` の load handler から `000start.json` が複数回fetchされ、surfaceManagerの事務所表示後にタイトル/ランダム表示系が再実行されていた。v038_23では surface takeover中の初期 `loadScenario(currentScenario)` を停止し、起動表示を surfaceManager へ完全委譲する。`000start.json` 自体も安全なoffice指定のみに変更。
