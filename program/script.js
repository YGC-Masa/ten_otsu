...
/* v037_79 engine guard: 起動停止対策 */
window.TENOTSU_ENGINE_VERSION = "v037_79";
...
function handleMenuAction(item) {
  ...
  /* v037_79 custom action precheck */
...
clickLayer.addEventListener("dblclick", (event) => {
  // v037_79: 左メニューはダブルクリックではなく長押しで出す。
  event.preventDefault();
});

let lastTouch = 0;
clickLayer.addEventListener("touchend", () => {
  // v037_79: ダブルタップメニューは廃止。長押しメニューへ統一。
  lastTouch = Date.now();
});

clickLayer.addEventListener("click", () => {
  /* v037_79: 右メニュー非表示時タップ再表示 */
...
/* v037_79 expose engine functions */
...
/* v037_79 boot flow: 起動フラッシュ → 初期化 → タイトル表示 → 事務所6大メニュー */
window.TENOTSU_BOOT_FLOW_VERSION = "v037_79";
...
/* v037_79 economy/status/album helpers */
...
/* v037_79 rank/equipment/unlock helpers */
...
/* v037_79 economy action listener */
document.addEventListener("click", (event) => {
  const btn = event.target.closest("[data-engine-action]");
  if (!btn) return;
  const action = btn.dataset.engineAction;
...
/* v037_79 manager level/EXP helpers */
...
/* v037_79 outer ADV / encounter / affection / item helpers */
...
/* v037_79: 左メニューをダブルクリックからクリックプレス/タップホールドへ変更 */
...
/* v037_79: 「タイトルに戻る」後メニューをメンバー配下へ移設 */
...
/* v037_79: タイトルタイル表示後も右メニューを6大メニュー固定 */
...
/* v037_79 randomImagesOn office6 reassert wrapper */
window.addEventListener("load", () => {
  window.setTimeout(() => {
