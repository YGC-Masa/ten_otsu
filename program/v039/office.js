...
(function(){
  ...
  ns.renderOfficeMenu = function renderOfficeMenu() {
    ...
    versionMain.textContent = ns.VERSION || window.TENOTSU_BUILD_VERSION || "v039_132 正式背景整理＋総合運動公園不足メモ";
    const versionSub = document.createElement("span");
    versionSub.className = "tenotsu-menu-version-sub";
    versionSub.textContent = ns.BUILD_LABEL || window.TENOTSU_BUILD_LABEL || "v039_132 正式背景整理＋総合運動公園不足メモ";
  ...
  ns.renderSettings = function renderSettings() {
    ...
    const html = `
      ...
      <div class="tenotsu-settings-body">
        <div>現在のバージョン: <strong>${ns.VERSION || window.TENOTSU_BUILD_VERSION || "v039_132 正式背景整理＋総合運動公園不足メモ"}</strong></div>
