...
(function(){
  ...
  ns.renderOfficeMenu = function renderOfficeMenu() {
    ...
    versionMain.textContent = ns.VERSION || window.TENOTSU_BUILD_VERSION || "v039_139 夜空メイン40CG範囲調整＋イベントCG終了時クリア";
    const versionSub = document.createElement("span");
    versionSub.className = "tenotsu-menu-version-sub";
    versionSub.textContent = ns.BUILD_LABEL || window.TENOTSU_BUILD_LABEL || "v039_139 夜空メイン40CG範囲調整＋イベントCG終了時クリア";
  ...
  ns.renderSettings = function renderSettings() {
    ...
    const html = `
      ...
      <div class="tenotsu-settings-body">
        <div>現在のバージョン: <strong>${ns.VERSION || window.TENOTSU_BUILD_VERSION || "v039_139 夜空メイン40CG範囲調整＋イベントCG終了時クリア"}</strong></div>
