...
/* v037_60 engine guard: 起動停止対策 */
window.TENOTSU_ENGINE_VERSION = "v037_60";
...
/* v037_60 expose engine functions */
...
/* v037_60 boot flow: 起動フラッシュ → 初期化 → タイトル表示 → 事務所6大メニュー */
window.TENOTSU_BOOT_FLOW_VERSION = "v037_60";
...
/* v037_60 economy/status/album helpers */
...
/* v037_60 rank/equipment/unlock helpers */
...
/* v037_60 economy action listener */
document.addEventListener("click", (event) => {
  const btn = event.target.closest("[data-engine-action]");
  if (!btn) return;
  const action = btn.dataset.engineAction;
...
/* v037_60 manager level/EXP helpers */
...
/* v037_60 outer ADV / encounter / affection / item helpers */
