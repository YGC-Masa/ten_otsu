...
(function () {
  ...
  function onStaffDoubleTap(staffId) {
    ...
    if (s.skill < 100) {
      // v037_14: ダブルタップ時に必殺不可でも、通常接客が可能なら通常クリックとして処理する
  ...
  function renderCutinOverlay() {
    ...
    return `
      <div class="battle-cutin ${state.cutin.image ? "with-image" : ""}" style="--cutin-color:${state.cutin.color}; opacity:${opacity};">
        ${state.cutin.image ? `<div class="battle-cutin-photo-surface"><img src="${escapeHtml(state.cutin.image)}" alt=""></div>` : ""}
