...
(function () {
  ...
  function onStaffDoubleTap(staffId) {
    ...
    if (s.skill < 100) {
      // v037_22: ダブルタップ時に必殺不可でも、通常接客が可能なら通常クリックとして処理する
  ...
  function useManagerHelp(fromAuto = false) {
    ...
    if (state.helpStock <= 0) {
      state.lastActionText = "店長ヘルプのストックがありません。通常クリック処理として案内表示します。";
      render();
      return false;
    }
  ...
  function handleHelpClick(event) {
    ...
    const now = performance.now();
    const isDoubleTap = pendingHelpTapAt && now - pendingHelpTapAt <= DOUBLE_TAP_MS;
    pendingHelpTapAt = now;

    if (isDoubleTap) {
      pendingHelpTapAt = 0;
      const used = useManagerHelp();
      if (!used) {
        state.lastActionText = "店長HELPはまだ発動できません。成約10件で1ストックです。";
        render();
      }
      return;
    }

    state.lastActionText = state.helpStock > 0
      ? "店長HELP待機中。ダブルクリック/ダブルタップで発動します。"
      : "店長HELP準備中。成約10件で1ストック溜まります。";
    render();
  }
    ...
    root.innerHTML = `
      <div class="battle-stage ${state.running ? "is-running" : ""} ${state.rush ? "is-rush" : ""}" style="--battle-bg-url: url(${getBattleBackground(state.battleBgId).path});">
        <section class="battle-hud">
          ...
          ${state.finished ? `<div class="battle-finished-banner">営業終了</div>` : state.timeSaleActive ? `<div class="battle-timesale-banner">＞＞＞タイムセール実施中！＜＜＜</div>` : ""}
        </section>
  ...
  document.addEventListener("click", (event) => {
    ...
    else if (action === "help") handleHelpClick(event);
  });
