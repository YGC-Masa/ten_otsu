...
(function () {
  ...
  function onStaffDoubleTap(staffId) {
    ...
    if (s.skill < 100) {
      // v037_17: ダブルタップ時に必殺不可でも、通常接客が可能なら通常クリックとして処理する
  ...
  function renderCutinOverlay() {
    ...
    return `
      <div class="battle-cutin ${state.cutin.image ? "with-image split-surface" : ""}" style="--cutin-color:${state.cutin.color}; opacity:${opacity};">
        ${state.cutin.image ? `
          <div class="battle-cutin-portrait-surface">
            <img src="${escapeHtml(state.cutin.image)}" alt="">
          </div>
        ` : ""}
        <div class="battle-cutin-band battle-cutin-text-surface">
