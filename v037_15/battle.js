...
(function () {
  ...
  function onStaffDoubleTap(staffId) {
    ...
    if (s.skill < 100) {
      // v037_15: ダブルタップ時に必殺不可でも、通常接客が可能なら通常クリックとして処理する
    ...
    return `
      <div class="battle-cutin ${state.cutin.image ? "with-image" : ""}" style="--cutin-color:${state.cutin.color}; opacity:${opacity};">
        <div class="battle-cutin-band">
          ...
          ${state.cutin.image ? `<div class="battle-cutin-photo-fixed"><img src="${escapeHtml(state.cutin.image)}" alt=""></div>` : ""}
        </div>
      </div>
    `;
  }
